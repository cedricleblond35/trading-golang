...
func NewCandleCollector(ctx context.Context, pdb *database.GORM, xtbConn *xtb.XTB) *CandleCollector {
...
func (cc *CandleCollector) Collected(symbol string) error {
	...
	for {
		...
		case message := <-messageOut:
			...
			err = insertData(respData, message.Period, cc.pdb)
			if err != nil {
				return err
			}
...
func insertData(resp ResponseChartLastRequest, period int, db *database.GORM) error {
	...
	for _, ligne := range resp.ReturnData.RateInfos {
		x := model.NewCandle()
		if err := db.LoadLast(x, "period = ? AND ctm =?", period, ligne.Ctm/1000); errors.Is(err, gorm.ErrRecordNotFound) {
			open := ligne.Open / 100
